    ...
    <main class="container px-4 py-8 mx-auto lg:flex md:gap-4 xl:gap-12 lg:py-12">
        ...
        <div class="lg:w-2/3">
            ...
            <div class="mb-8 bg-white rounded-3xl">
                ...
                <!-- Galeri Foto -->
                @if ($destinasiwisata->photo && count($destinasiwisata->photo) > 0)
                <div class="my-8">
                    <h2 class="mb-6 text-2xl font-bold text-gray-800">Galeri Foto</h2>
                    <div class="grid grid-cols-2 gap-4 md:grid-cols-3">
                        @foreach ($destinasiwisata->photo as $foto)
                            @php
                                // Cek apakah sudah full URL atau hanya path relatif
                                $fotoUrl = Str::startsWith($foto->photo, ['http://', 'https://'])
                                    ? $foto->photo
                                    : asset('storage/' . $foto->photo);
                            @endphp

                            <div class="relative overflow-hidden cursor-pointer rounded-2xl gallery-image group"
                                onclick="openModal('{{ $fotoUrl }}')">

                                <img src="{{ $fotoUrl }}" alt="{{ $destinasiwisata->nama }}"
                ...
                {{-- <div class="space-y-4">
                    @foreach ($destinasiwisatalain as $item)
                        <a href="{{ route('fe.wisata.detail', $item->slug) }}" class="flex items-center space-x-3">
                            <img src="{{ asset('storage/' . $item->photo->first()->photo) }}"
                                alt="Foto {{ $item->nama }}"
